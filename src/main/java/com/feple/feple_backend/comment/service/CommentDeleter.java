 ...
 * deleteByPostIds는 벌크 DELETE라 @SQLDelete를 우회한 하드 삭제이고, deleteSingle은 엔티티 단건
 * remove라 Comment의 @SQLDelete에 가로채여 실제로는 소프트 삭제(deleted_at 세팅)로 처리된다 —
 * comment.parent_id가 자기참조 FK(RESTRICT)라 대댓글이 달린 댓글을 하드 삭제하면 FK 위반이 나기
 * 때문에 의도된 동작이다. CommentServiceImpl(게시글 삭제에 딸린 일괄 삭제)과
