...
public class WebScrapeAdminController {
    ...
    public ResponseEntity<?> scrape(@RequestBody Map<String, String> body) {
        ...
        if (url == null || url.isBlank()) {
            return AdminErrorResponses.badRequest("URL을 입력해주세요.");
        }
        try {
            ...
            return AdminErrorResponses.serverError("페이지를 가져오는데 실패했습니다. URL을 확인하거나 직접 입력해주세요.");
        }
    }
    ...
    public ResponseEntity<?> applyScrape(@RequestBody ScraperApplyRequestDto req) {
        ...
        if (validationError != null) return AdminErrorResponses.badRequest(validationError);
        try {
            ...
            return AdminErrorResponses.serverError("페스티벌 등록에 실패했습니다.");
        }
    }
