...
public class FestivalCsvExporter {
    ...
    public String buildCsv() {
        return CsvExporter.buildCsv("ID,제목,영어제목,지역,장소,시작일,종료일,좋아요,참석의사\n",
                festivalAdminService.getAllFestivalsForAdmin(),
                f -> new Object[]{
                        f.getId(), f.getTitle(), f.getTitleEn(),
                        f.getRegion() != null ? f.getRegion().getDisplayName() : "",
                        f.getLocation(), f.getStartDateIso(), f.getEndDateIso(),
                        f.getLikeCount(), f.getAttendingCount() });
    }
}
