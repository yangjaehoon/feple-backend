...
class ArtistAdminControllerTest {
    ...
    void 목록_조회_뷰와_모델_속성_확인() throws Exception {
        given(artistService.getAdminArtistList(anyString(), anyString(), any(), anyInt()))
    ...
    void 장르_필터_있으면_getAdminArtistList에_전달() throws Exception {
        given(artistService.getAdminArtistList(anyString(), anyString(), eq(ArtistGenre.INDIE), anyInt()))
                .willReturn(new PageImpl<>(List.of()));
        given(artistSuggestionAdminService.getPendingSuggestionsPreview(anyInt())).willReturn(List.of());
        given(artistSuggestionAdminService.getProcessedSuggestionsPreview(anyInt())).willReturn(List.of());
        given(artistSuggestionAdminService.countProcessed()).willReturn(0L);
        ...
        then(artistService).should().getAdminArtistList(anyString(), anyString(), eq(ArtistGenre.INDIE), anyInt());
    }
    ...
    void 편집_폼_조회_성공() throws Exception {
        given(artistService.getArtistForEdit(1L)).willReturn(mock(ArtistRequestDto.class));
    ...
    void 편집_폼_조회_없는_아티스트_목록으로_리다이렉트() throws Exception {
        given(artistService.getArtistForEdit(99L)).willThrow(new NoSuchElementException("없는 아티스트"));
    ...
    void 아티스트_수정_NoSuchElementException_errorMessage_설정() throws Exception {
        willThrow(new NoSuchElementException("없는 아티스트")).given(artistService).updateArtist(anyLong(), any());
    ...
    void 아티스트_삭제_성공_successMessage_없음() throws Exception {
        ...
        then(artistService).should().deleteArtist(1L);
    }

    @Test
    void 아티스트_삭제_실패_errorMessage_설정() throws Exception {
        willThrow(new RuntimeException("삭제 실패")).given(artistService).deleteArtist(anyLong());
    ...
    void 영어_이름_일괄_수정_성공() throws Exception {
        ...
        then(artistService).should().batchUpdateNameEn(List.of(1L, 2L), List.of("Artist A", "Artist B"));
    }
}
